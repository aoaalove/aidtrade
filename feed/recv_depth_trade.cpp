...
boost::lockfree::spsc_queue<nlohmann::json, boost::lockfree::capacity<1024>> _queue;
Config _config;
		...
		while (!_queue.empty())
		{
			...
			if (stream == "depth5@100ms")  {
				...
				if (_config.print_depth)
					print_depth_all(symbol, book, _config.depth_count);
			} else {
				...
				if (event == "trade") {
					...
					if (_config.print_trade)
					{
						std::cout << "[TRADE] " << symbol << " " 
							...
							<< std::setw(16) << std::right << price
							<< std::setw(16) << std::right << qty
							<< std::endl;
					}
				}
			}
